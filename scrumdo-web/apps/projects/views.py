...
# If they have no organization, it suggests they create one or ask to join
# one.
def home(request):
    my_projects = []
    member_projects = []
...
def search_project(request, group_slug):
    ...
    if search_terms == "":
        search_results = SearchQuerySet().filter(
            project_id=project.id).models(Story).order_by("rank").load_all()
    else:
        search_results = SearchQuerySet().filter(project_id=project.id).filter(
            content=search_terms).models(Story).order_by("rank").load_all()
...
# The project admin page, this is where you can change the title,
# description, etc. of a project.
@login_required
def project_admin(request, group_slug):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get("action") == "moveToOrganization":
            ...
            messages.info(request, "Project moved to organization")
        ...
        #         project.save()
        #         messages.info(request, "Project removed from organization")
        if request.POST.get("action") == "add":
            write_access_or_403(project, request.user)
            adduser_form = AddUserForm(
                request.POST, project=project, user=request.user)
...
def iteration_burndown(request, group_slug, iteration_id):
    ...
    if iteration.start_date:
        start_date_timestamp = int(
            (time.mktime(iteration.start_date.timetuple()) - time.timezone) * 1000)
    if iteration.end_date:
        end_date_timestamp = int(
            (time.mktime(iteration.end_date.timetuple()) - time.timezone) * 1000)
    ...
    if iteration.start_date:
        total_points = filter(
            lambda x: x[0] >= start_date_timestamp, total_points)
        claimed_points = filter(
            lambda x: x[0] >= start_date_timestamp, claimed_points)
    if iteration.end_date:
        total_points = filter(
            lambda x: x[0] <= end_date_timestamp, total_points)
        claimed_points = filter(
            lambda x: x[0] <= end_date_timestamp, claimed_points)

    total_stats = {"label": "Total Points",
                   "data": reduce_burndown_data(total_points)}
    claimed_stats = {"label": "Claimed Points",
                     "data": reduce_burndown_data(claimed_points)}
...
# The project history page, which lets you see a burn up chart for each
# past iteration.
@login_required
...
def create(request, form_class=ProjectForm, template_name="projects/create.html"):
    ...
    if project_form.is_valid():
        ...
        if organization:
            creationAllowed = org_project_limit.increaseAllowed(
                organization=organization)
        else:
            creationAllowed = personal_project_limit.increaseAllowed(
                user=request.user)
        ...
        if creationAllowed:
            ...
            # And lets make the default backlog iteration with no start/end
            # dates.
            default_iteration = Iteration(
                name='Backlog', detail='', default_iteration=True, project=project)
            project.iterations.add(default_iteration)
            default_iteration.save()

            messages.info(request, "Project Created")
            # Finished successfully creating a project, send the user to that
            # page.
...
def addProjectToOrganization(project, organization):
    logger.info("Adding project %s to organization %s" %
                (project.slug, organization.slug))
    project.organization = organization
    project.save()
    admin_teams = organization.teams.filter(access_type="staff")
    admin_team = admin_teams[0]
    if not admin_team:
    # This really shouldn't happen, there's no way to delete that default
    # admin team.
...
def delete(request, group_slug=None, redirect_url=None):
    ...
    project.delete()
    messages.info(request, _("Project %(project_name)s deleted.") %
...
def your_projects(request, template_name="projects/your_projects.html"):
    projects = Project.objects.filter(
        member_users=request.user).order_by("name")
...
@login_required
def project(
    request, group_slug=None, form_class=ProjectUpdateForm, adduser_form_class=AddUserForm,
    ...
    return render_to_response(template_name, {
        ...
        "add_story_form": add_story_form,
        "project": project,
        # @@@ this should be the only context var for the project
        "group": project,
        "is_member": is_member,
...
def project_prediction(request, group_slug):
    project = get_object_or_404(Project, slug=group_slug)
    read_access_or_403(project, request.user)
    stories = project.get_default_iteration().stories.exclude(
        status=Story.STATUS_DONE).order_by("rank")
    ...
    for story in stories:
        if story.points_value() > points_left:
            record_prediction(
                predictions, temp_stories, iteration_number, start_date, iteration_length, points_left)
            iteration_number += 1
            if carry_over:
                points_left += velocity
            else:
    ...
    record_prediction(predictions, temp_stories,
                      iteration_number, start_date, iteration_length, points_left)
...
def record_prediction(predictions, stories, iteration_number, start_date, iteration_length, points_left):
    ...
    predictions.append(
        {"carried": points_left, "stories": stories, "points": points, "num": iteration_number,
         "start": start_date, "end": (start_date + datetime.timedelta(days=(iteration_length - 1)))})
