...
def organization_dashboard(request, organization_slug):
    ...
    favorite_projects = Favorite.objects.filter(user=request.user, project__organization=organization).select_related(
        'project').order_by("-project__active", "project__category", "project__name")
...
def organization_projects(request, organization_slug):
    ...
    # have been consolodated, this post is now from the organization overview
    # page
        ...
        if action == "addMember":
            ...
            if not request.POST.get("project", None):
                messages.info(request,
                              "Please select a project to add to this team.")
            else:
            ...
            project = Project.objects.filter(
                id=request.POST.get("project_id"))[0]
            ...
            if user == request.user and team.access_type == "staff":
                messages.info(request,
                    "Can't remove yourself from the staff group.")
            else:
    ...
    projects = organization.projects.all().order_by(
...
def organization_edit(request, organization_slug):
    ...
    if request.method == "POST" and organization.creator == request.user:
        ...
        if form.is_valid():
            messages.info(request, "Organization Updated")
            form.save()
        else:
            messages.info(request,
...
def handle_organization_create(form, request, projects):
    ...
    member_team = Team(
        organization=organization, name="Members", access_type="write")
    member_team.save()

    staff_team = Team(
        organization=organization, name="Staff", access_type="staff")
    staff_team.save()
    staff_team.members.add(request.user)

    messages.info(request, "Organization Created.")
    ...
    signals.organization_created.send(
        sender=request, organization=organization)
...
def delete_organization(request, organization_slug):
    ...
    if not organization.hasStaffAccess(request.user):
        raise PermissionDenied()
    signals.organization_deleted.send(
        sender=request, organization=organization)
    for project in organization.projects.all():
