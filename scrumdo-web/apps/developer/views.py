...
def apply(request):
    ...
    if request.method == 'POST':
        if form.is_valid():  # All validation rules pass
            ...
            dev_key.save()
            messages.info(request, "Created developer key.")
...
def user_keys(request):
    if request.method == 'POST' and request.POST.get("key_id"):
        ...
        messages.info(request,
            "Deleted developer key for application %s." % appname)
...
# this must match the templates found in tutorial/ . ie
# tutorial/name_of_tutorial/url_of_page.html
