...
def your_bookmarks(request):
    bookmark_instances = BookmarkInstance.objects.filter(
        user=request.user).order_by("-saved")
...
def add(request):
    ...
    if request.method == "POST":
        ...
        if bookmark_form.is_valid():
            ...
            try:
                ...
                req = urllib2.Request(bookmark.get_favicon_url(
                    force=True), None, headers)
            ...
            if bookmark_form.should_redirect():
                ...
                messages.info(request,
                              _("You have saved bookmark '%(description)s'") % {'description': bookmark_instance.description})
    ...
    bookmarks_add_url = "http://" + \
        Site.objects.get_current().domain + reverse(add)
...
def delete(request, bookmark_instance_id):
    ...
    bookmark_instance = get_object_or_404(
        BookmarkInstance, id=bookmark_instance_id)
