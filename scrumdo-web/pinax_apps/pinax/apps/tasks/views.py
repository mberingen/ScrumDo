...
def add_task(request, group_slug=None, secret_id=None, form_class=TaskForm, template_name="tasks/add.html", bridge=None):
    ...
    if secret_id and Snippet:
        ...
        # Update the expiration time to maximum
        paste.expires = datetime.now() + timedelta(
            seconds=3600 * 24 * 30 * 12 * 100)
        paste.save()
    ...
    if request.method == "POST":
        if request.user.is_authenticated():
            ...
            if task_form.is_valid():
                ...
                task.save()
                task.save_history()
                messages.info(request, "added task '%s'" % task.summary)
                if notification:
                    if group:
                    ...
                    notify_list = notify_list.exclude(
                        id__exact=request.user.id)
                    notification.send(notify_list, "tasks_new", {
                                      "creator": request.user, "task": task, "group": group})
...
def nudge(request, id, group_slug=None, bridge=None):
    ...
    if notification:
        ...
        notification.send(notify_list, "tasks_nudge", {
                          "nudger": request.user, "task": task, "count": count})
...
def task(request, id, group_slug=None, template_name="tasks/task.html", bridge=None):
    ...
    if is_member and request.method == "POST":
        ...
        if form.is_valid():
            ...
            if "status" in form.changed_data:
                messages.info(request, "updated your status on the task")
                if notification:
                    notification.send(notify_list, "tasks_status", {
                                      "user": request.user, "task": task, "group": group})
            if "state" in form.changed_data:
                messages.info(request, "task marked %s" %
                              task.get_state_display())
                if notification:
                    notification.send(notify_list, "tasks_change", {
                                      "user": request.user, "task": task, "group": group, "new_state": task.get_state_display()})
            if "assignee" in form.changed_data:
                messages.info(request, "assigned task to '%s'" % task.assignee)
                if notification:
                    notification.send(notify_list, "tasks_assignment", {
                                      "user": request.user, "task": task, "assignee": task.assignee, "group": group})
            if "tags" in form.changed_data:
                messages.info(request, "updated tags on the task")
                if notification:
    ...
    if is_member and request.user != task.assignee and task.assignee:
        ...
        try:
            nudge['nudge'] = Nudge.objects.filter(
                nudger__exact=request.user, task__exact=task)[0]
        except IndexError:
            nudge['nudge'] = None
...
def user_tasks(request, username, group_slug=None, template_name="tasks/user_tasks.html", bridge=None):
    ...
    if group:
        other_user = get_object_or_404(
            group.member_queryset(), username=username)
        group_base = bridge.group_base_template()
    else:
    ...
    assigned_filter = TaskFilter(
        filter_data, queryset=assigned_tasks, prefix="a")
    created_filter = TaskFilter(
        filter_data, queryset=created_tasks, prefix="c")

    assigned_tasks = assigned_filter.qs
    created_tasks = created_filter.qs

    assigned_tasks = assigned_tasks.order_by(
        "state", "-modified")  # @@@ filter(project__deleted=False)
    # @@@ filter(project__deleted=False)
    created_tasks = created_tasks.order_by("state", "-modified")
...
def mini_list(request, group_slug=None, template_name="tasks/mini_list.html", bridge=None):
    ...
    assigned_tasks = request.user.assigned_tasks.all().exclude(
        state="2").exclude(state="3").order_by("state", "-modified")
...
def focus(request, field, value, group_slug=None, template_name="tasks/focus.html", bridge=None):
    ...
    if field == "modified":
        try:
            ...
            tasks = tasks.filter(
                modified__year=int(year), modified__month=int(month), modified__day=int(day))
        except:
        ...
        # @@@ this means can't have a username 'unassigned':
        if value == "unassigned":
        ...
        try:
            ...
            tasks = tasks.filter(id__in=Tag.objects.get(name=value).items.filter(
                content_type=task_type).values_list("object_id", flat=True))
...
def tasks_history(request, id, group_slug=None, template_name="tasks/task_history.html", bridge=None):
    ...
    for change in task_history:
        change.humanized_state = workflow.STATE_CHOICES_DICT.get(
            change.state, None)
        change.humanized_resolution = workflow.RESOLUTION_CHOICES_DICT.get(
