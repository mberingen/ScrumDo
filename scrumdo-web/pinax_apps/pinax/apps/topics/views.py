        ...
        if request.user.is_authenticated():
            if is_member:
                ...
                if topic_form.is_valid():
                    ...
                    topic.save()
                    messages.info(
                        request, _("You have started the topic %(topic_title)s") %
                        {"topic_title": topic.title})
                    # @@@ is this the right way to reset it?
                    topic_form = form_class()
            else:
                messages.info(
                    request, _("You are not a member and so cannot start a new topic"))
                topic_form = form_class()
        else:
