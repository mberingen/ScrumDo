...
def delete(request, group_slug=None, redirect_url=None):
    ...
    if (request.user.is_authenticated() and request.method == "POST" and
        ...
        tribe.delete()
        messages.info(request, _("Tribe %(tribe_name)s deleted.") %
...
def tribe(request, group_slug=None, form_class=TribeUpdateForm,
    ...
    if action == 'update' and tribe_form.is_valid():
        ...
        if not is_member:
            ...
            messages.info(request,
                          _("You have joined the tribe %(tribe_name)s") % {"tribe_name": tribe.name})
            is_member = True
            if notification:
                notification.send([tribe.creator], "tribes_created_new_member", {
                                  "user": request.user, "tribe": tribe})
                notification.send(
                    tribe.members.all(), "tribes_new_member", {"user": request.user, "tribe": tribe})
        else:
            messages.info(request,
                          _("You have already joined tribe %(tribe_name)s") % {"tribe_name": tribe.name})
    elif action == 'leave':
        tribe.members.remove(request.user)
        messages.info(request, "You have left the tribe %(tribe_name)s" %
                      {"tribe_name": tribe.name})
        is_member = False
        if notification:
    ...
    return render_to_response(template_name, {
        "tribe_form": tribe_form,
        "tribe": tribe,
        # @@@ this should be the only context var for the tribe
        "group": tribe,
        "is_member": is_member,
    }, context_instance=RequestContext(request))
