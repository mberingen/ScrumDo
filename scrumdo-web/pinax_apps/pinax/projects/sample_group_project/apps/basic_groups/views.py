...
def create(request, form_class=BasicGroupForm, template_name="basic_groups/create.html"):
    ...
    if group_form.is_valid():
        ...
        if notification:
            ...
            notification.send(
                User.objects.all(), "groups_new_group", {"group": group}, queue=True)
...
def delete(request, group_slug=None, redirect_url=None):
    ...
    if request.user.is_authenticated() and request.method == "POST" and request.user == group.creator and group.members.all().count() == 1:
        group.delete()
        messages.info(request, "Group %s deleted." % group)
...
def group(request, group_slug=None, form_class=BasicGroupUpdateForm, template_name="basic_groups/group.html"):
    ...
    if action == "update" and group_form.is_valid():
        ...
        group.members.add(request.user)
        messages.info(request, "You have joined the group %s" % group.name)
        if notification:
            notification.send([group.creator], "groups_created_new_member", {
                              "user": request.user, "group": group})
            notification.send(
                group.members.all(), "groups_new_member", {"user": request.user, "group": group})
    elif action == "leave":
        group.members.remove(request.user)
        messages.info(request, "You have left the group %s" % group.name)
        if notification:
