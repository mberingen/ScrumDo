...
def friends(request, form_class=JoinRequestForm,
        ...
        if request.POST["action"] == "accept":
            try:
                ...
                if invitation.to_user == request.user:
                    invitation.accept()
                    messages.info(
                        request, _("Accepted friendship request from %(from_user)s") %
            ...
            try:
                ...
                if invitation.to_user == request.user:
                    invitation.decline()
                    messages.info(
                        request, _("Declined friendship request from %(from_user)s") %
                        {'from_user': invitation.from_user})
            except FriendshipInvitation.DoesNotExist:
                pass
            join_request_form = form_class()
    ...
    invites_received = request.user.invitations_to.invitations().order_by(
        "-sent")
    invites_sent = request.user.invitations_from.invitations().order_by(
        "-sent")
...
def accept_join(request, confirmation_key, form_class=SignupForm,
    ...
    join_invitation = get_object_or_404(
        JoinInvitation, confirmation_key=confirmation_key.lower())
    if request.user.is_authenticated():
        return render_to_response("account/signup.html", {
        }, context_instance=RequestContext(request))
    else:
        form = form_class(
            initial={"email": join_invitation.contact.email, "confirmation_key": join_invitation.confirmation_key})
        return render_to_response(template_name, {
            "form": form,
        }, context_instance=RequestContext(request))
    ...
    if request.method == "POST":
        if request.POST["action"] == "upload_vcard":
            ...
            if import_vcard_form.is_valid():
                ...
                messages.info(
                    request, _("%(total)s vCards found, %(imported)s contacts imported.") %
            ...
            if request.POST["action"] == "import_yahoo":
                ...
                if bbauth_token:
                    ...
                    messages.info(
                        request, _("%(total)s people with email found, %(imported)s contacts imported.") %
                        {'imported': imported, 'total': total})
            if request.POST["action"] == "import_google":
                authsub_token = request.session.get('authsub_token')
                del request.session['authsub_token']
                if authsub_token:
                    imported, total = import_google(
                        authsub_token, request.user)
                    messages.info(
                        request, _("%(total)s people with email found, %(imported)s contacts imported.") %
                        {'imported': imported, 'total': total})
    else:
