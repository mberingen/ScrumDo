...
def proposed_by_you(request):
    swaps = Swap.objects.filter(
        proposing_offer__offerer=request.user, state=1).order_by("-proposed_time")
...
def proposed_to_you(request):
    swaps = Swap.objects.filter(
        responding_offer__offerer=request.user, state=1).order_by("-proposed_time")
    ...
    if request.method == "POST":
        if request.POST["action"] == "create":
            ...
            if offer_form.is_valid():
                ...
                offer.save()
                messages.info(request, _("Successfully saved offer '%s'") %
...
def edit_offer(request, offer_id):
    ...
    if offer.offerer != request.user:
        messages.info(request, "You cannot edit offers that are not yours")
        return HttpResponseRedirect(reverse("offer_list_yours"))
            ...
            if offer_form.is_valid():
                ...
                if notification:
                    for swap in offer.proposed_swaps.filter(state=1):
                        notification.send(
                            [swap.responding_offer.offerer,
                             ], "swaps_proposing_offer_changed",
                    ...
                    for swap in offer.responding_swaps.filter(state=1):
                        notification.send(
                            [swap.proposing_offer.offerer,
                             ], "swaps_responding_offer_changed",
                            {"creator": request.user,
                             "swap": swap,
                             "proposing_offer": swap.proposing_offer,
                             "responding_offer": swap.responding_offer})

                messages.info(request, _("Successfully updated offer '%s'") %
                              offer.short_description)
...
def delete_offer(request, offer_id):
    offer = get_object_or_404(Offer, id=offer_id)
    if offer.offerer != request.user:
        messages.info(request, "You cannot delete offers that are not yours")
        return HttpResponseRedirect(reverse("offer_list_yours"))
    if request.method == "POST":
        offer.delete()
    return HttpResponseRedirect(reverse("offer_list_yours"))
...
def cancel_offer(request, offer_id):
    offer = get_object_or_404(Offer, id=offer_id)
    if offer.offerer != request.user:
        messages.info(request, "You cannot cancel offers that are not yours")
        return HttpResponseRedirect(reverse("offer_list_yours"))
    if request.method == "POST":
        offer.cancel()
    return HttpResponseRedirect(reverse("offer_list_yours"))
...
def propose_swap(request, offer_id):
    ...
    if request.method == "POST":
        ...
        swap_form.fields['proposing_offer'].queryset = Offer.objects.filter(
            offerer=request.user, state=1)
...
def cancel_swap(request, swap_id):
    ...
    if notification:
        notification.send(
            [swap.responding_offer.offerer, ], "swaps_cancellation",
            {"creator": request.user,
             "swap": swap,
             "proposing_offer": swap.proposing_offer,
             "responding_offer": swap.responding_offer})
