...
class ProjectStories(Feed):
    ...
    def item_pubdate(self, item):
        # Returning the time the action was created, lets RSS readers sort them
        # properly.
        return item.created
    ...
    def item_guid(self, obj):
        # We need to return unique GUIDs for each activity, or RSS readers will
        # assume they're the same entry
        return "GUID-%d" % obj.id
...
def getIterationsStories(iterations):
    ...
    for iteration in iterations:
        activities.extend(
            StoryActivity.objects.filter(story__iteration=iteration))
        activities.extend(
            IterationActivity.objects.filter(iteration=iteration))
    print activities
    ...
    def get_object(self, project_and_iteration):
        ...
        if project.token == project_and_iteration[1]:
            iteration = get_object_or_404(
                Iteration, pk=project_and_iteration[2])
